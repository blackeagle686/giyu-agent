...
function updateReports(generations) {
    ...
    generations.forEach((gen, index) => {
        ...
        if (typeof gen === 'string') {
            content = gen;
        } else if (gen.artifacts && Array.isArray(gen.artifacts)) {
            // Handle complex generation blocks (like giyu-generator)
            content = `### <i class="bi bi-gear-wide-connected"></i> Logic Generation\n\n`;
            gen.artifacts.forEach(art => {
                if (art.type === 'file_write') {
                    content += `**File:** \`${art.path}\` (${art.language})\n\`\`\`${art.language}\n${art.code}\n\`\`\`\n\n`;
                } else if (art.type === 'terminal') {
                    content += `**Terminal Command:**\n\`\`\`bash\n${art.code}\n\`\`\`\n\n`;
                }
            });
            if (gen.metadata?.model) {
                content += `*Generated by ${gen.metadata.model}*`;
            }
        } else {
            content = gen.text || gen.content || JSON.stringify(gen, null, 2);
        }

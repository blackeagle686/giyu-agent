"""
Giyu Agent — Main Full-Screen Chat UI.

Layout:
  ┌─ Header ─────────────────────────────────────────────────────┐
  │ 🐦‍🔥 GIYU  │  model name  │  status badge                  │
  ├─ Body ──────────────────────────────────────────────────────┤
...
class ThinkingSpinner(Static):
    ...
    def show(self, label: str = "Giyu is thinking") -> None:
        self._label = label
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes=f"msg-content-container {role_class}"):
            ...
            if self.message.role == "user":
                ...
                yield Label(f"GIYU  •  {self.message.timestamp}", classes="msg-header")
...
class ChatScreen(Screen):
    ...
    async def _init_agent_worker(self) -> None:
        try:
            ...
            self._agent = await get_giyu_agent()
    ...
    def _start_stream(self, user_text: str, mode: str) -> None:
        ...
        self.query_one("#thinking-spinner", ThinkingSpinner).show("Giyu is thinking")
